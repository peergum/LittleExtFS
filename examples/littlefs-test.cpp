...
SerialLogHandler logHandler(
    LOG_LEVEL_WARN,
    {
        ...
        {"app",
         LOG_LEVEL_INFO}  // Default logging level for all application messages
    });
...
SpiFlashWinbond spiFlash(SPI, A5);  // Winbond flash on SPI (D pins)
...
// typical config for E series
// SpiFlashP1 spiFlash;					// P1 external flash inside the
// P1 module
...
const size_t BLOCK_SIZE = 16384;  // This affects RAM usage.
...
// The maximum number of files was determined by seeing when writes fail because
// the volume was full. There is more overhead than I would have thought.
...
// MX25L25645G = 256 Mbit = 32 Mbyte: NUM_FILES = 96, PHYS_SIZE = 32 * 1024 *
// 1024
const size_t NUM_FILES = 96;
const size_t PHYS_SIZE = 16UL * 1024 * 1024;

LittleExtFS fs(&spiFlash, 0, PHYS_SIZE / 4096U, 4096U);
...
class LogTime {
 public:
  ...
  inline ~LogTime() { Log.info("finished %s: %lu ms", desc, millis() - start); }
...
void stateChipEraseAndMount() {
  ...
  // Unmount if mounted
  LittleExtFS::getInstance()->unmount();
  ...
  res = LittleExtFS::getInstance()->mount();
...
void stateStartNextFile() {
  ...
  fd = lefs_open(fileName, O_RDWR | O_CREAT);
  if (fd == -1) {
...
void stateWriteAndTest() {
  ...
  Log.info("WR Block %d", blockNum);
  fillBuf(testBuf1, sizeof(testBuf1));

  size_t offset = curOffset;

  int writeResult = lefs_write(fd, testBuf1, sizeof(testBuf1));
  if (writeResult != sizeof(testBuf1)) {
    Log.info("lefs_write failure %d != %u at offset %u", writeResult,
             sizeof(testBuf1), offset);
    lefs_close(fd);
    stateHandler = stateFailure;
    return;
  }
  ...
  // Log.info("Seek");
  // lefs_lseek(fd, offset, SEEK_SET);

  // Log.info("RD Block %d", blockNum);
  // lefs_read(fd, (char *)testBuf2, sizeof(testBuf2));
...
void stateVerifyBlocks() {
  ...
  if (memcmp(testBuf1,testBuf2, sizeof(testBuf1))) {
    Log.info("mismatched data blockNum=%u",
             blockNum);
  }
  // for (size_t ii = 0; ii < sizeof(testBuf1); ii++) {
  //   if (testBuf1[ii] != testBuf2[ii]) {
