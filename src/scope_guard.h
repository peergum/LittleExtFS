 ...
 * to another function:
 * ```
 * int openResourceFile(lefs_t *lefs, lefs_file_t* file, const char* name) {
 *     // Open the resource file
 *     auto result = lefs_file_open(lefs, file, name, LEFS_O_RDONLY);
 *     if (result < 0) {
 ...
 *         lefs_file_close(lefs, file);
 *     });
 *     // Check the header signature
 *     const headerSize = 3;
 *     char header[headerSize + 1] = {}; // Reserve one character for the term. null
 *     result = lefs_file_read(lefs, file, header, headerSize);
 *     if (result < 0) {
